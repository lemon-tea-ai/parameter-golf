...
class Hyperparameters:
    ...
    curriculum_switch_frac = float(os.environ.get("CURRICULUM_SWITCH_FRAC", 0.5))
...
class CastedLinear(nn.Linear):
    def forward(self, x: Tensor) -> Tensor:
        w = self.weight
        clip = getattr(self, "_qat_clip", 0)
        if clip > 0 and self.training and w.ndim == 2:
            w_f = w.float()
...
class GPT(nn.Module):
    def __init__(
        ...
        self.aux_head = CastedLinear(model_dim, vocab_size, bias=False) if aux_loss_weight > 0 else None
        if self.aux_head is not None:
            self.aux_head._zero_init = True
        self._init_weights()
    ...
    def forward(self, input_ids: Tensor, target_ids: Tensor) -> Tensor:
        ...
        if self.training and self.aux_head is not None and self.aux_loss_weight > 0:
            aux_x = x[:, :-1, :].reshape(-1, x.size(-1))
            aux_targets = target_ids[:, 1:].reshape(-1)
            aux_logits = self.aux_head(aux_x)
            aux_logits = self.logit_softcap * torch.tanh(aux_logits / self.logit_softcap)
            loss = loss + self.aux_loss_weight * F.cross_entropy(aux_logits.float(), aux_targets, reduction="mean")
        return loss
...
def main() -> None:
    ...
    use_curriculum = args.curriculum_short_seq > 0 and args.curriculum_short_seq < args.train_seq_len
    if args.warmup_steps > 0:
        initial_model_state = {name: tensor.detach().cpu().clone() for name, tensor in base_model.state_dict().items()}
        initial_optimizer_states = [copy.deepcopy(opt.state_dict()) for opt in optimizers]
        model.train()
        half_warmup = args.warmup_steps // 2
        for warmup_step in range(args.warmup_steps):
            ws_seq = args.curriculum_short_seq if (use_curriculum and warmup_step < half_warmup) else args.train_seq_len
            zero_grad_all()
            for micro_step in range(grad_accum_steps):
                if distributed:
                    model.require_backward_grad_sync = micro_step == grad_accum_steps - 1
                x, y = train_loader.next_batch(args.train_batch_tokens, ws_seq, grad_accum_steps)
                with torch.autocast(device_type="cuda", dtype=torch.bfloat16, enabled=True):
            ...
            if args.warmup_steps <= 20 or (warmup_step + 1) % 10 == 0 or warmup_step + 1 == args.warmup_steps:
                log0(f"warmup_step:{warmup_step + 1}/{args.warmup_steps} seq_len:{ws_seq}")
    ...
    while True:
        ...
        if use_curriculum and max_wallclock_ms and elapsed_ms < max_wallclock_ms * args.curriculum_switch_frac:
            current_seq_len = args.curriculum_short_seq
        else:
    ...
    # Discard aux head — only used during training, not in the artifact
