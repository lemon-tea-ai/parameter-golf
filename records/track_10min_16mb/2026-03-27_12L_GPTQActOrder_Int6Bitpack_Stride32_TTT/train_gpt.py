...
class Hyperparameters:
    ...
    ttt_grad_clip = float(os.environ.get("TTT_GRAD_CLIP", 1.8))
...
# --- Int6 bitpacking (4 values → 3 bytes, 25% smaller than int8) ---

def pack_int6(q: Tensor) -> Tensor:
    """Pack int8 tensor with values in [-32,31] into 6-bit packed uint8."""
    flat = q.reshape(-1)
    n = flat.numel()
    u = (flat.int() + 32).clamp(0, 63).to(torch.uint8)
    pad = (4 - n % 4) % 4
    if pad:
        u = torch.cat([u, torch.zeros(pad, dtype=torch.uint8, device=u.device)])
    u4 = u.reshape(-1, 4)
    packed = torch.empty(u4.shape[0], 3, dtype=torch.uint8, device=u4.device)
    packed[:, 0] = (u4[:, 0] << 2) | (u4[:, 1] >> 4)
    packed[:, 1] = ((u4[:, 1] & 0xF) << 4) | (u4[:, 2] >> 2)
    packed[:, 2] = ((u4[:, 2] & 0x3) << 6) | (u4[:, 3] & 0x3F)
    return packed.reshape(-1)

def unpack_int6(packed: Tensor, n_orig: int) -> Tensor:
    """Unpack 6-bit packed uint8 back to int8 with values in [-32,31]."""
    data = packed.reshape(-1, 3)
    u0 = (data[:, 0] >> 2) & 0x3F
    u1 = ((data[:, 0] & 0x3) << 4) | (data[:, 1] >> 4)
    u2 = ((data[:, 1] & 0xF) << 2) | (data[:, 2] >> 6)
    u3 = data[:, 2] & 0x3F
    u = torch.stack([u0, u1, u2, u3], dim=1).reshape(-1)
    return (u[:n_orig].int() - 32).to(torch.int8)

# --- GPTQ ActOrder calibration and quantization ---

def collect_layer_hessians(base_model: nn.Module, val_tokens: Tensor, device: torch.device,
                           num_samples: int = 256, seq_len: int = 512) -> dict[str, Tensor]:
    """Collect input Hessians H = X^T X for each layer's weight matrices via block hooks."""
    n = base_model.num_layers
    # accumulators: keyed by (slot, layer_idx)
    h_attn: dict[int, Tensor] = {}   # input to Q/K/V/Out: attn_norm output, dim=model_dim
    h_mlp_up: dict[int, Tensor] = {} # input to MLP up: mlp_norm output, dim=model_dim
    h_mlp_dn: dict[int, Tensor] = {} # input to MLP down: after leaky_relu^2, dim=mlp_dim
    handles: list = []

    # We capture inside Block.forward by monkey-patching temporarily
    orig_forwards = {}
    model_dim = base_model.tok_emb.weight.shape[1]

    def make_capture_forward(block, idx, orig_fwd):
        def capturing_forward(x, x0, q_w, k_w, v_w, out_w, up_w, down_w, v_embed=None, v0=None):
            with torch.no_grad():
                mix = block.resid_mix.to(dtype=x.dtype)
                x_in = mix[0][None, None, :] * x + mix[1][None, None, :] * x0
                ai = block.attn_norm(x_in) * block.ln_scale_factor  # [B,T,D]
                flat = ai.reshape(-1, ai.size(-1)).float()
                if idx not in h_attn:
                    h_attn[idx] = torch.zeros(flat.size(1), flat.size(1), device=device)
                h_attn[idx].addmm_(flat.T, flat)

                # Attention output to get MLP input
                attn_out, _ = block.attn(ai, q_w, k_w, v_w, out_w, v_embed=v_embed, v0=v0)
                x_after = x_in + block.attn_scale.to(dtype=x_in.dtype)[None, None, :] * attn_out
                mi = block.mlp_norm(x_after) * block.ln_scale_factor
                flat_m = mi.reshape(-1, mi.size(-1)).float()
                if idx not in h_mlp_up:
                    h_mlp_up[idx] = torch.zeros(flat_m.size(1), flat_m.size(1), device=device)
                h_mlp_up[idx].addmm_(flat_m.T, flat_m)

                # MLP down input = leaky_relu(up @ x).square()
                hidden = F.leaky_relu(F.linear(mi, up_w.to(mi.dtype)), negative_slope=0.5).square()
                flat_d = hidden.reshape(-1, hidden.size(-1)).float()
                if idx not in h_mlp_dn:
                    h_mlp_dn[idx] = torch.zeros(flat_d.size(1), flat_d.size(1), device=device)
                h_mlp_dn[idx].addmm_(flat_d.T, flat_d)

            return orig_fwd(x, x0, q_w, k_w, v_w, out_w, up_w, down_w, v_embed=v_embed, v0=v0)
        return capturing_forward

    for i, block in enumerate(base_model.blocks):
        orig_forwards[i] = block.forward
        block.forward = make_capture_forward(block, i, block.forward)

    base_model.eval()
    total = val_tokens.numel() - 1
    step = max(1, total // num_samples // seq_len)
    with torch.inference_mode():
        for s in range(0, min(num_samples * step * seq_len, total - seq_len), step * seq_len):
    ...
    # Return hessians keyed by name matching unbanked state_dict
    result: dict[str, Tensor] = {}
    for i in range(n):
        for slot in [f"blocks.{i}.attn.c_q.weight", f"blocks.{i}.attn.c_k.weight",
                     f"blocks.{i}.attn.c_v.weight", f"blocks.{i}.attn.proj.weight"]:
            if i in h_attn:
                result[slot] = h_attn[i]
        if i in h_mlp_up:
            result[f"blocks.{i}.mlp.fc.weight"] = h_mlp_up[i]
        if i in h_mlp_dn:
            result[f"blocks.{i}.mlp.proj.weight"] = h_mlp_dn[i]
    return result


def quantize_gptq_actorder(W: Tensor, H: Tensor, clip_range: int = 31,
                            block_size: int = 128, percdamp: float = 0.01) -> tuple[Tensor, Tensor]:
    """GPTQ with ActOrder: Hessian-sorted column quantization with error propagation.
    W: [out_features, in_features], H: [in_features, in_features].
    Returns (q_int8, scale_fp16) where scale is per-row."""
    try:
        n_rows, n_cols = W.shape
        if H.shape[0] != n_cols or H.shape[1] != n_cols:
        ...
        # Compute per-row scales from original weight (before ordering)
        row_max = W.abs().amax(dim=1).clamp_min(1.0 / clip_range)
        S = (row_max / clip_range).to(torch.float32)  # [out_features]

        # Cholesky of H^{-1} (upper triangular) for efficient block propagation
        try:
            H_inv = torch.linalg.cholesky(H)
            H_inv = torch.cholesky_inverse(H_inv)
            H_inv = torch.linalg.cholesky(H_inv, upper=True)
        except Exception:
            # Cholesky failed — fall back to naive per-row int6
            return quantize_int6_per_row(W[:, perm.argsort()], clip_range)

        Q = torch.zeros_like(W)

        for i in range(0, n_cols, block_size):
            i2 = min(i + block_size, n_cols)
            W1 = W[:, i:i2].clone()
            ...
            for j in range(i2 - i):
                w = W1[:, j]
                d = H1[j, j]
                q = torch.clamp(torch.round(w / S), -clip_range, clip_range)
                Q[:, i + j] = q
                err = (w - q * S) / d
                W1[:, j:] -= err.unsqueeze(1) * H1[j:j + 1, j:]
                Err1[:, j] = err

            if i2 < n_cols:
                W[:, i2:] -= Err1 @ H_inv[i:i2, i2:]

        # Unsort: put columns back in original order
        Q_orig = torch.zeros_like(Q)
        Q_orig[:, perm] = Q
        return Q_orig.to(torch.int8), S.to(torch.float16)

    except Exception:
        return quantize_int6_per_row(W if W.ndim == 2 else W, clip_range)
...
def mixed_quantize_int6(state_dict: dict[str, Tensor], int6_cats: set[str],
    ...
    for name, tensor in state_dict.items():
        ...
        if cat in int6_cats and t.ndim >= 1:
            ...
            n_orig = q.numel()
            packed = pack_int6(q)
            result[name + ".q6"] = packed
            result[name + ".q6_shape"] = torch.tensor(list(q.shape), dtype=torch.int32)
            result[name + ".scale"] = s
            meta[name] = {"type": "int6bp"}  # int6 bitpacked
        else:
            q, s = quantize_float_tensor(t)
            result[name + ".q"] = q
            result[name + ".scale"] = s
...
def dequantize_mixed_int6(result: dict[str, Tensor], meta: dict[str, object],
    ...
    for name, orig in template_sd.items():
        ...
        if info in ("passthrough", "passthrough_ctrl", "passthrough_fp16"):
            ...
            out[name] = t
            continue
        if isinstance(info, dict) and info.get("type") == "int6bp":
            shape = result[name + ".q6_shape"].tolist()
            n_orig = 1
            for d in shape:
                n_orig *= d
            q = unpack_int6(result[name + ".q6"], n_orig).reshape(shape)
            s = result[name + ".scale"]
...
def main() -> None:
    ...
    # Collect GPTQ calibration hessians (rank 0 only, then broadcast)
    hessians: dict[str, Tensor] | None = None
    if args.gptq_actorder:
        log0(f"gptq_actorder:collecting hessians samples={args.gptq_calib_samples}")
        torch.cuda.synchronize()
        t_hess = time.perf_counter()
        hessians = collect_layer_hessians(
            base_model, val_tokens, device,
            num_samples=args.gptq_calib_samples, seq_len=args.train_seq_len,
        )
        torch.cuda.synchronize()
    ...
    if master_process:
        ...
        code_bytes = len(code.encode("utf-8"))
        log0(f"Serialized model int6bp+{_COMPRESSOR}: {quant_file_bytes} bytes")
        log0(f"Total submission size int6bp+{_COMPRESSOR}: {quant_file_bytes + code_bytes} bytes")
    if distributed:
        dist.barrier()
